...
$cronToken = trim((string) Env::get('GSC_SYNC_CRON_TOKEN', ''));
...
if (!$isCli) {
    if ($cronToken === '') {
        ...
        echo json_encode([
            'success' => false,
            'error' => 'GSC_SYNC_CRON_TOKEN is not configured.',
        ]);
        exit;
    }
...
if ($isCli && isset($argv[1])) {
    $limit = max(1, min(2000, (int) $argv[1]));
...
$result = $service->syncAllConnectedProjects($limit);
...
file_put_contents($storageDir . '/search_console_sync.log', $logLine . PHP_EOL, FILE_APPEND | LOCK_EX);
$usageMonitoring->logCronExecution('search-console-sync', true, [
    'processed' => (int) ($result['processed'] ?? 0),
    'failed' => (int) ($result['failed'] ?? 0),
